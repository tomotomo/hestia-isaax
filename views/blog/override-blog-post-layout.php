...
class Hestia_Blog_Post_Layout {
	...
	public function init() {
		add_action( 'override_blog_post_template_part', array( $this, 'render' ), 2 );
	}
	...
	private function render_post_thumbnail( $type = 'default' ) {
		...
		if ( has_post_thumbnail() ) {
            $post_thumbnail = get_the_post_thumbnail( null, $size );
        } else {
            // 無いときはNo image出す
			$post_thumbnail = sprintf(
                '<img src="%s" alt="">',
                get_stylesheet_directory_uri() . '/assets/img/no-image.png'
            );
        }
		...
		$post_thumbnail_content .= $post_thumbnail;
